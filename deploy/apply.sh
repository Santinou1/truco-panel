...
[[ "$run_id" =~ ^[0-9]+$ ]]
environment="$(sed -n 's/^DEPLOY_ENVIRONMENT=//p' deploy.env)"
network="$(sed -n 's/^DEPLOY_NETWORK=//p' deploy.env)"
case "$environment" in
  testing)
    [[ "$base" == /opt/pulperia/testing/truco-panel && "$network" == pulperia-testing-web ]]
    grep -Fxq 'DEPLOY_PROJECT=pulperia-panel-testing' deploy.env
    ;;
  production)
    [[ "$base" != */testing/* && "$network" == pulperia-web ]]
    grep -Fxq 'DEPLOY_PROJECT=pulperia-panel' deploy.env
    ;;
...
docker network inspect "$network" >/dev/null 2>&1 || docker network create "$network" >/dev/null || docker network inspect "$network" >/dev/null
